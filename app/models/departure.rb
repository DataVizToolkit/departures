  ...
  def self.timeline
    sql = <<-SQL.strip_heredoc
      WITH hours AS (
        ...
        SELECT id, year, month, day_of_month, dep_time, arr_time,
          (year::text || LPAD(month::text, 2, '0') ||
            ...
            LPAD(dep_time::text, 4, '0'))::timestamp AS departure_time,
          (year::text || LPAD(month::text, 2, '0') ||
            LPAD(day_of_month::text, 2, '0') || ' ' ||
            LPAD(arr_time::text, 4, '0'))::timestamp AS arrival_time,
      ...
      SELECT ROW_NUMBER() OVER (ORDER BY hourly) AS row_id, hourly, id,
        ...
        actual_elapsed_time, origin, dest, distance,
        COALESCE(tail_num, 'NA')
      FROM flights
      RIGHT JOIN hours ON
        tsrange(hours.hourly, hours.hourly + '1 hour') @> departure_time
      WHERE tsrange('1999-01-01 00:00'::timestamp,
                    '1999-01-01 23:59'::timestamp) @> hourly
    SQL
